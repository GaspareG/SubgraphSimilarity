...
long long fpaths = 0ll;
long long fpathsc = 0ll;
...
bool colorful(vector<int> p)
{
  bool *visto = new bool[q];
  for(int i=0; i<(int)q; q++) visto[i] = false;
  for(int u : p)
  {
    if( visto[ color[u] ] ) return false;
    visto[ color[u] ] = true;
  }
  return true;
}
...
void dfs(int t, int u, int k) {
  ...
  P[t].push_back(u);
  //Pcolor[t] = setBit(Pcolor[t], color[u]);
  ...
  if (k == 0) {
    #pragma omp critical
    {
      fpaths++;
      COLORSET cs = 0ll;
      for(int l : P[t] ) cs = setBit(cs, color[l]);
      if( cs == ( (1<<q)-1 ) ) fpathsc++;
  ...
  Pstring[t].pop_back();
  P[t].pop_back();
  //Pcolor[t] = clearBit(Pcolor[t], color[u]);
...
vector<int> naiveRandomPathTo(int u) {
  ...
  for (int i = q - 1; i > 0; i--) {
    ...
    vector<ll> freqX;
    freqX.clear();
    for (int x : Nu) freqX.push_back(MF[q][x]);
    discrete_distribution<int> distribution(freqX.begin(), freqX.end());

    #pragma omp critical
    {
      u = Nu[ distribution(eng) ];
//      u = Nu[eng() % Nu.size()];
    }
    Ps.insert(u);
    P.push_back(u);
...
//set<string> baselineSampler(vector<int> X, int r) {
  set<vector<int>> R;
  long long ps = 0ll;
  long long cs = 0ll;
  ...
  vector<ll> freqX;
  freqX.clear();
  for (int x : X) freqX.push_back(MF[q][x]);
  discrete_distribution<int> distribution(freqX.begin(), freqX.end());
...
//        u = X[eng() % X.size()];
  ...
  for(auto P : R)
  {
    ...
    freqP[ label ]++;
    COLORSET s = 0ll;
    for(int l : P ) s = setBit(s, color[l]);
    if( s == ((1<<q)-1) ) freqC[label]++;
  }

  // vector< pair<double, string> > closest;
  // for(auto i : freqP)
  // {
  //   string s = i.first;
  //   double dist = ((double)freqC[s]/freqP[s]);
  //   dist = abs( dist - ratio );
  //   closest.push_back( make_pair( dist, s) );
  // }
  // sort(closest.begin(), closest.end());
  // set<string> ret;
  // for(int i=0; i<5; i++)
  // {
  //   string s = closest[i].second;
  //   reverse(s.begin(), s.end());
  //   ret.insert(s);
  //   printf("[%.6f] [%s] [%.6f]\n", ratio, s.c_str(), (double)freqC[s]/freqP[s]);
  // }
  // return ret;
//  for(auto i : freqP)
//  {
//    string s = i.first;
//    printf("\t[%4s] [%5lld][%5lld] [%.6f]\n", s.c_str(), freqP[s], freqC[s], (double)freqC[s]/freqP[s] );
//  }
//  printf("PATHS = %lld || C-PATHS = %lld\n", ps, cs);
    ...
    int main(int argc, char **argv) {
      ...
      for(unsigned int i = 0; i < experiment; i++)
      {
        ...
        if( baseline_flag )
        {
          ...
          // set<string> BLsampling = baselineSampler(X, R);
          // map<string,  BLsampling = baselineSampler(X, R);

          map<pair<int, string>, ll> BLsampling = baselineSampler(X, R);
          // freqA = processFrequency(BLsampling, multiset<int>(A.begin(), A.end()));
          // freqB = processFrequency(BLsampling, multiset<int>(B.begin(), B.end()));
